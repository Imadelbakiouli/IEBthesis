...
output_dir = 'feature_similarity/diff_grids_sequence_rowscale'
os.makedirs(output_dir, exist_ok=True)

for vid_idx in range(30):
    real_path = f'xai_results/selected_layers_sequence/video_{vid_idx:02d}/heatmaps_sequence_video{vid_idx:02d}.npy'
    styl_path = f'xai_results/selected_layers_sequence_stylized/video_{vid_idx:02d}/heatmaps_sequence_stylized_video{vid_idx:02d}.npy'
    ...
    for i, layer in enumerate(layers):
        ...
        for j, idx in enumerate(frame_indices):
            diff = real_h[layer][idx] - styl_h[layer][idx]
            axes[i, j].imshow(diff, cmap='RdBu_r', vmin=-row_vmax, vmax=row_vmax)
            axes[i, j].axis('off')
